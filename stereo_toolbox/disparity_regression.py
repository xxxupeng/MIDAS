...
class argmax_disparityregression(nn.Module):
    def __init__(self, maxdisp=192):
        super(argmax_disparityregression, self).__init__()
...
class unimodal_disparityregression_SA(nn.Module):
    def __init__(self, maxdisp=192):
        super(unimodal_disparityregression_SA, self).__init__()
        self.maxdisp = maxdisp

    def modal_mask(self, x):
        N, D, H, W = x.shape
    ...
    def forward(self, x):
        ...
        mask = self.modal_mask(x_blur)
        y = x * mask
        z = x - y
        x_blur = x_blur * (~mask)
        z = z * self.modal_mask(x_blur)
