...
class PWCNet(nn.Module):
    def __init__(self, maxdisp, use_concat_volume=True):
        ...
        self.disparityregression = mean_disparityregression(self.maxdisp)
    ...
    def forward(self, left, right):
        ...
        if self.training:
            ...
            pred0 = self.disparityregression(pred0)
            ...
            combine = self.disparityregression(combine)

            # disp = disparity_regression(pred3, self.maxdisp)
            ...
            # refinenet_feature_right_warp = warp(refinenet_feature_right, disp)
            ...
            # pred3feature = self.dispupsample(disp)
            # refinenet_combine = torch.cat((refinenet_feature_left - refinenet_feature_right_warp, refinenet_feature_left, pred3feature, disp, refinenet_costvolume), dim = 1)
            # disp_finetune = self.refinenet3(refinenet_combine, disp)
            # disp_finetune = torch.squeeze(disp_finetune, 1)

            return pred0, combine, pred1, pred2, pred3
