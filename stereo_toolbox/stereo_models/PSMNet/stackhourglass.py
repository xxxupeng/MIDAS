...
class PSMNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        # hints = hints.unsqueeze(1).unsqueeze(2).expand(-1, features, num_disp, -1, -1).detach()
        # validhints = validhints.unsqueeze(1).unsqueeze(2).expand(-1, features, num_disp, -1, -1).detach()

        # # create a tensor of the same size as cost, with disparities
        # # between 0 and num_disp-1 along the disparity dimension
        # disparities = torch.linspace(start=0, end=num_disp - 1, steps=num_disp).cuda().unsqueeze(0).unsqueeze(1).unsqueeze(3).unsqueeze(4).expand(batch_size, features, -1, height, width).detach()
